...
@Component
@org.springframework.core.annotation.Order(2) // Run after RoleSeeder
public class PermissionSeeder implements CommandLineRunner {
  ...
  private void seedPermissions() {
    ...
    List<Permission> allPermissionsList =
        Arrays.asList(
            ...
            createPermissionIfNotFound(
                "LOAN:READ_ALL", "Melihat semua pengajuan pinjaman (sesuai hak akses)"),
            ...
            createPermissionIfNotFound(
                "LOAN:LIST_PENDING_REVIEW", "Melihat daftar pending review (Marketing)"),
            createPermissionIfNotFound("LOAN:REVIEW", "Melakukan review pengajuan (Marketing)"),
            createPermissionIfNotFound(
                "LOAN:LIST_WAITING_APPROVAL", "Melihat daftar waiting approval (BM)"),
            createPermissionIfNotFound("LOAN:APPROVE", "Melakukan approval pengajuan (BM)"),
            createPermissionIfNotFound(
                "LOAN:LIST_WAITING_DISBURSE", "Melihat daftar waiting disbursement (Backoffice)"),
            createPermissionIfNotFound(
                "LOAN:DISBURSE", "Melakukan pencairan pinjaman (Backoffice)"),
            createPermissionIfNotFound(
                "LOAN:REJECT_BACKOFFICE", "Menolak pencairan pinjaman (Backoffice)"),
    ...
    List<String> commonBasePermissions = Arrays.asList("AUTH:LOGOUT", "AUTH:CHANGE_PASSWORD");
        ...
        getPermissionsSet(
            combine(
                ...
                Arrays.asList(
                    ...
                    "USER_PLAFOND:ASSIGN",
                    "USER_PLAFOND:READ",
                    "USER_PLAFOND:HISTORY",
                    "LOAN:READ_ALL",
                    "LOAN:DETAILS",
                    "LOAN:HISTORY",
                    // List permissions for viewing all workflow tabs
                    "LOAN:LIST_PENDING_REVIEW",
                    "LOAN:LIST_WAITING_APPROVAL",
                    "LOAN:LIST_WAITING_DISBURSE",
                    // Menu visibility
                    "MENU:DASHBOARD",
                    "MENU:USERS",
                    "MENU:BRANCH",
