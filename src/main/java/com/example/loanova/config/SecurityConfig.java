 ...
 *
 * <p>Fungsi file ini: 1. Enable Spring Security untuk seluruh aplikasi (@EnableWebSecurity) 2.
 * Enable method-level security untuk @PreAuthorize (@EnableMethodSecurity) 3. Configure URL mana
 * yang public, mana yang perlu authentication 4. Setup JWT filter untuk validate token di setiap
 * request 5. Set session jadi STATELESS (no server-side session, pakai JWT)
 */
@Configuration
...
public class SecurityConfig {
  ...
  /** Bean untuk AuthenticationManager Dipakai di AuthService untuk authenticate user saat login */
  @Bean
   ...
   *
   * <p>Method ini define: - URL mana yang public (permitAll) - URL mana yang perlu authentication -
   * Filter apa yang dipakai - Session management
   */
  @Bean
  public SecurityFilterChain filterChain(HttpSecurity http, JwtAuthenticationFilter jwtAuthFilter)
    ...
    http
        ...
        // ADD JWT FILTER - Filter ini jalan sebelum Spring Security check
        // authentication
