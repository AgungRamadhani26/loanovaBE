...
/** LOAN APPLICATION SERVICE - Menangani logika bisnis pengajuan dan proses pinjaman. */
@Service
...
public class LoanApplicationService {
  ...
  /**
   * SUBMIT LOAN APPLICATION - Customer mengajukan pinjaman baru Logic: 1. Validasi user profile
   * sudah lengkap 2. Validasi tidak ada pinjaman aktif 3. Validasi plafond dan amount 4. Validasi
   * tenor 5. Snapshot data dari user profile 6. Upload dokumen 7. Kurangi remaining amount 8.
  ...
  public LoanApplicationResponse submitLoanApplication(
    ...
    Branch branch =
        branchRepository
            ...
            .orElseThrow(
                () ->
                    new ResourceNotFoundException("Branch tidak ditemukan dengan ID: " + branchId));
    ...
    if (tenor < plafond.getTenorMin() || tenor > plafond.getTenorMax()) {
      throw new BusinessException(
          "Tenor harus antara "
              + plafond.getTenorMin()
              + " - "
    ...
    try {
      ...
      String savingBookCoverPath =
          fileStorageUtil.saveFile(request.getSavingBookCover(), "loan-documents");
      String payslipPhotoPath =
          fileStorageUtil.saveFile(request.getPayslipPhoto(), "loan-documents");
      ...
      userPlafond.setRemainingAmount(userPlafond.getRemainingAmount().subtract(amount));
  ...
  public LoanApplicationResponse getApplicationDetail(String username, Long applicationId) {
    ...
    LoanApplication application =
        loanApplicationRepository
            ...
            .orElseThrow(() -> new ResourceNotFoundException("Loan application tidak ditemukan"));
  ...
  @Transactional(readOnly = true)
  public List<ApplicationHistoryResponse> getApplicationHistory(
    ...
    if (user.getRoles().stream()
        .anyMatch(
            r ->
                r.getRoleName().equalsIgnoreCase("SUPERADMIN")
    ...
    return applicationHistoryRepository
        .findByLoanApplicationOrderByCreatedAtDesc(application)
        .stream()
  ...
  /**
   * GET PENDING APPLICATIONS FOR MARKETING - Marketing melihat list pinjaman dengan status
  ...
  public List<LoanApplicationResponse> getPendingApplicationsForMarketing(String username) {
    ...
    return loanApplicationRepository
        .findByStatusAndBranch(
            LoanApplicationStatus.PENDING_REVIEW.name(), user.getBranch().getId())
        .stream()
        .map(this::toResponse)
        .collect(Collectors.toList());
  }
  ...
  public LoanApplicationResponse reviewByMarketing(
    ...
    if (request.getAction().equalsIgnoreCase("PROCEED")) {
      ...
      userPlafond.setRemainingAmount(userPlafond.getRemainingAmount().add(application.getAmount()));
      userPlafondRepository.save(userPlafond);
  ...
  /**
   * GET ALL APPLICATIONS - Get all loan applications based on user role SUPERADMIN/BACKOFFICE: See
   * ALL MARKETING/BRANCHMANAGER: See Branch Only CUSTOMER: See Own Only
   */
  @Transactional(readOnly = true)
  public List<LoanApplicationResponse> getAllApplications(String username) {
    ...
    if (roles.contains("SUPERADMIN") || roles.contains("BACKOFFICE")) {
      ...
      applications =
          loanApplicationRepository.findByBranchIdOrderBySubmittedAtDesc(user.getBranch().getId());
    } else {
  ...
  /**
   * GET WAITING APPROVAL APPLICATIONS FOR BRANCH MANAGER - Branch Manager melihat list pinjaman
  ...
  /** APPROVE BY BRANCH MANAGER - Branch Manager melakukan approval (APPROVE/REJECT) */
  @Transactional
  ...
  /**
   * GET WAITING DISBURSEMENT APPLICATIONS FOR BACKOFFICE - Backoffice melihat semua pinjaman dengan
  ...
  public LoanApplicationResponse disburseByBackoffice(String username, Long applicationId) {
    ...
    notificationService.createNotification(
        application.getUser(),
        "Dana Pinjaman Cair!",
        "Kabar gembira! Dana pinjaman Anda sebesar Rp "
  ...
  @Transactional
  public LoanApplicationResponse rejectByBackoffice(
      String username, Long applicationId, LoanReviewRequest request) {
    User user =
        userRepository
            .findByUsername(username)
  ...
  /**
   * Helper method untuk copy file snapshot dari user profile ke folder loan-snapshots Tujuan:
