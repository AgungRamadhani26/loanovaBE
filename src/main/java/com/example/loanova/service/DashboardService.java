...
/**
 * DASHBOARD SERVICE
 * Menangani logika bisnis untuk statistik dashboard
...
public class DashboardService {
    ...
    public DashboardStatisticsResponse getStatistics(String username) {
        ...
        boolean isGlobalRole = user.getRoles().stream()
        ...
        // Calculate amounts
        BigDecimal totalSubmissionAmount = applications.stream()
            .map(LoanApplication::getAmount)
            .reduce(BigDecimal.ZERO, BigDecimal::add);

        List<LoanApplication> disbursedApps = applications.stream()
            .filter(app -> "DISBURSED".equals(app.getStatus()))
            .collect(Collectors.toList());

        BigDecimal totalDisbursedAmount = disbursedApps.stream()
            .map(LoanApplication::getAmount)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    /**
     * Calculate total interest from disbursed loans
     * Formula: amount * (interestRate/100) * tenor
    ...
    private BigDecimal calculateTotalInterest(List<LoanApplication> disbursedApps) {
        return disbursedApps.stream()
            .map(app -> {
                ...
                BigDecimal rate = app.getInterestRateSnapshot() != null 
                    ? app.getInterestRateSnapshot() : BigDecimal.ZERO;
        ...
        return ALL_STATUSES.stream()
            .map(status -> {
                ...
                Double percentage = totalCount > 0 
                    ? (count.doubleValue() / totalCount) * 100 
                    : 0.0;
    ...
    private List<PlafondDistribution> calculatePlafondDistribution() {
        ...
        Map<String, Long> plafondCounts = activePlafonds.stream()
            .collect(Collectors.groupingBy(
                up -> up.getPlafond().getName(),
                Collectors.counting()
            ));

        return plafondCounts.entrySet().stream()
            .map(entry -> PlafondDistribution.builder()
