 ...
 *
 * <p>JWT (JSON Web Token) terdiri dari 3 bagian: 1. Header: Algorithm & token type 2. Payload:
 * Claims (data user, expiration, dll) 3. Signature: Verify token tidak diubah
 *
 * <p>Fungsi service ini: 1. Generate access token (15 menit) 2. Generate refresh token (7 hari) 3.
 * Validate token (signature, expiration, username) 4. Extract data dari token (username, claims,
 * expiration)
 */
@Service
public class JwtService {
   ...
   *
   * <p>Access token: - Durasi: 15 menit - Dipakai di header Authorization untuk setiap request -
   ...
   *
   * <p>Custom claims = data tambahan yang mau disimpan di JWT Contoh: email, fullName, userId,
   * permissions, dll
   *
   * <p>PERHATIAN: - Jangan simpan data sensitif (password, credit card) - JWT bisa di-decode (not
   * encrypted, only signed) - Semakin banyak claims, semakin besar size token
   *
   ...
   *
   * <p>Refresh token: - Durasi: 7 hari - Disimpan di database untuk bisa di-revoke - Dipakai di
   * endpoint /api/auth/refresh untuk generate access token baru - Lebih aman karena bisa di-track &
   * revoke
   *
   * @param userDetails User yang login
   ...
   *
   * <p>JWT Structure: Header.Payload.Signature
   *
   * <p>Payload (Claims): - extraClaims: custom data (roles, email, dll) - subject: username/email
   * user - issuedAt: waktu token dibuat - expiration: waktu token expired
   *
   * <p>Signature: - Sign dengan HMAC-SHA256 + secret key - Verify token tidak diubah
   *
  ...
  private String generateToken(
    ...
    return Jwts.builder()
        ...
        .expiration(
            new Date(
                System.currentTimeMillis() + expiration)) // exp = expiration (registered claim)
   ...
   *
   * <p>Validation checks: 1. Signature: apakah token di-sign dengan secret key kita? 2. Username:
   * apakah username di token = username di DB? 3. Expiration: apakah token sudah expired?
   *
   ...
   *
   * <p>Username disimpan di claim "subject" (sub)
   *
   * @param token JWT token string
   ...
   *
   * <p>Claims = data yang disimpan di JWT payload Contoh claims: - subject (sub): username -
   * expiration (exp): waktu expired - issuedAt (iat): waktu dibuat - custom claims: email, roles,
   * dll
   *
   * @param token JWT token string
   ...
   *
   * <p>Process: 1. Parse JWT string 2. Verify signature dengan secret key 3. Return payload
   * (claims)
   *
   * <p>Exception: - SignatureException: kalau signature invalid (token diubah) -
   * ExpiredJwtException: kalau token sudah expired - MalformedJwtException: kalau format token
   * salah
   *
   * @param token JWT token string
   ...
   *
   * <p>Process: 1. Convert secret string ke byte array dengan UTF-8 encoding 2. Create SecretKey
   * untuk HMAC-SHA256 algorithm
   *
   * <p>PENTING: - Secret HARUS minimal 256-bit (32 karakter UTF-8) - Secret disimpan plain text di
   * application.properties (NOT Base64) - Di production: pakai environment variable!
   *
