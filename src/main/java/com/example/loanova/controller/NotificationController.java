...
public class NotificationController {
  ...
  public ResponseEntity<ApiResponse<String>> testPushNotification(
      @org.springframework.web.bind.annotation.RequestBody
          com.example.loanova.dto.request.PushNotificationRequest request) {

    com.example.loanova.entity.User user =
        userRepository
            .findByUsername(request.getUsername())
            .orElseThrow(
                () ->
                    new com.example.loanova.exception.ResourceNotFoundException(
                        "User tidak ditemukan"));

    notificationService.createNotification(user, request.getTitle(), request.getMessage());

    return ResponseUtil.ok(
        "Notifikasi berhasil dikirim ke user: " + request.getUsername(), "Success");
  }
}
